...
export function createGame(): GameState {
  const s: GameState = {
    phase: 'ready', wave: 1, lives: START_LIVES, gold: START_GOLD, score: 0, killed: 0,
    units: [], snake: [], headD: 0, shop: [], rerollCost: 10,
    pendingEvo: null, pendingEvent: null, resume: 'ready',
    goldMul: 1, speedMul: 1, fx: { popups: [], beams: [] }, time: 0, nextId: 1,
  }
  rollShop(s)
  return s
}
...
export const freeSlots = (s: GameState): number[] => SLOTS.map((_, i) => i).filter((i) => !unitAt(s, i))
...
export function startWave(s: GameState): void {
  ...
  s.snake = makeSnake(s.wave, s.nextId)
...
export function chooseEvolution(s: GameState, choice: 'safe' | 'risky'): boolean {
  ...
  s.phase = s.resume
  if (!u) return false
  if (choice === 'safe') { u.evo = 'safe'; return true }
  if (rand() < 0.5) { u.evo = 'risky'; return true }
