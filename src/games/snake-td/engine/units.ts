...
export const UNIT_DEFS: Record<UnitType, UnitDef> = {
  volt:   { type: 'volt',   name: 'Вольт',  color: '#ffd54a', price: 15, dmg: 1, rate: 3,   range: 120, desc: 'Быстрые удары' },
  frost:  { type: 'frost',  name: 'Фрост',  color: '#5bc8ff', price: 20, dmg: 2, rate: 1.2, range: 130, desc: 'Замедляет змею' },
  blaze:  { type: 'blaze',  name: 'Блейз',  color: '#ff5a5a', price: 25, dmg: 3, rate: 0.9, range: 110, desc: 'Урон по соседям' },
  venom:  { type: 'venom',  name: 'Веном',  color: '#4de08a', price: 25, dmg: 2, rate: 1,   range: 125, desc: 'Яд, урон со временем' },
  shadow: { type: 'shadow', name: 'Шэдоу', color: '#8d7bff', price: 35, dmg: 7, rate: 0.5, range: 170, desc: 'Дальний мощный удар' },
}
...
export function unitRange(u: Unit): number {
  return UNIT_DEFS[u.type].range + (u.level - 1) * 6
}
